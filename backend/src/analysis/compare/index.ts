 ...
 */
export function formatComparisonMarkdown(comparison: AnalysisComparison): string {
  const lines: string[] = [];
  lines.push('# Analysis Comparison');
  lines.push('');
  lines.push(`| | Source A | Source B |`);
  lines.push(`|---|---|---|`);
  lines.push(`| **ID** | ${comparison.sourceA.id} | ${comparison.sourceB.id} |`);
  lines.push(`| **Name** | ${comparison.sourceA.name} | ${comparison.sourceB.name} |`);
  lines.push(`| **Analyzed** | ${comparison.sourceA.analyzedAt} | ${comparison.sourceB.analyzedAt} |`);
  lines.push('');
  lines.push(`## ${comparison.summary}`);
  lines.push('');

  // Group by direction
  const improved = comparison.deltas.filter((d) => d.direction === 'improved');
  const regressed = comparison.deltas.filter((d) => d.direction === 'regressed');
  const unchanged = comparison.deltas.filter((d) => d.direction === 'unchanged');
  ...
  if (improved.length > 0) {
    ...
    for (const d of improved) {
      lines.push(`| ${d.metric} | ${fmtVal(d.before)} | ${fmtVal(d.after)} |`);
    }
    lines.push('');
  }
  ...
  if (unchanged.length > 0) {
    ...
    for (const d of unchanged) {
      lines.push(`| ${d.metric} | ${fmtVal(d.before)} |`);
    }
    lines.push('');
  }
